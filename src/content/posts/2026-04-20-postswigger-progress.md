...
If you're new to web security it's hard to know where to begin, so I've grouped every topic into three tracks — server-side, client-side, and advanced — and work through them in that order, completing the labs as I go. Within each track the ordering follows [onyxwizard's roadmap](https://github.com/onyxwizard/portswigger-academy). For more guided support, PortSwigger's own [learning paths](https://portswigger.net/web-security/learning-paths) are a good companion.
